...
__global__ void GINTint2e_jk_kernel(JKMatrix jk, BasisProdOffsets offsets,
    ...
    GINTkernel_getjk<NROOTS>(envs, bpcache, jk, g, ish, jsh, ksh, lsh, igroup);
}
...
__global__ void GINTint2e_jk_kernel(JKMatrix jk, BasisProdOffsets offsets,
    ...
    GINTg0_2e_2d4d<I, J, K, L>(envs, bpcache, g, u, w, norm, as_ish, as_jsh,
        as_ksh, as_lsh, ij, kl, iroot);
    GINTkernel_getjk<I, J, K, L>(
        envs, bpcache, jk, g, ish, jsh, ksh, lsh, igroup);
}

template <>
__global__ void GINTint2e_jk_kernel<2, 24>(JKMatrix jk,
    ...
    for (int iroot = 0; iroot < 2; iroot++) {
        GINTrys_root<2>(x, u, w, iroot);
        GINTg0_2e_2d4d<2>(envs, bpcache, g, u, w, norm, as_ish, as_jsh, as_ksh,
            as_lsh, ij, kl, iroot);
        GINTkernel_getjk<2>(envs, bpcache, jk, g, ish, jsh, ksh, lsh, igroup);
    }
}

template <>
__global__ void GINTint2e_jk_kernel<3, 72>(JKMatrix jk,
    BasisProdOffsets offsets, GINTEnvVars envs, BasisProdCache bpcache) {
    int ntasks_ij = offsets.ntasks_ij;
    long ntasks = ntasks_ij * offsets.ntasks_kl;
    long task_ij = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    for (int iroot = 0; iroot < 3; iroot++) {
        GINTrys_root<3>(x, u, w, iroot);
        GINTg0_2e_2d4d<3>(envs, bpcache, g, u, w, norm, as_ish, as_jsh, as_ksh,
            as_lsh, ij, kl, iroot);
        GINTkernel_getjk<3>(envs, bpcache, jk, g, ish, jsh, ksh, lsh, igroup);
    }
}

__global__ static void GINTint2e_jk_kernel0000(JKMatrix jk,
    BasisProdOffsets offsets, GINTEnvVars envs, BasisProdCache bpcache) {
    int ntasks_ij = offsets.ntasks_ij;
    long ntasks = ntasks_ij * offsets.ntasks_kl;
    long task_ij = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    for (i_dm = 0; i_dm < n_dm; ++i_dm) {
        if (vj != NULL) {
            reduce(gout0 * dm[i0 + nao * j0], vj + k0 + nao * l0);
            reduce(gout0 * dm[k0 + nao * l0], vj + i0 + nao * j0);
            vj += nao2;
        }
        if (vk != NULL) {
            reduce(gout0 * dm[j0 + nao * l0], vk + i0 + nao * k0);
            reduce(gout0 * dm[j0 + nao * k0], vk + i0 + nao * l0);
            reduce(gout0 * dm[i0 + nao * l0], vk + j0 + nao * k0);
            reduce(gout0 * dm[i0 + nao * k0], vk + j0 + nao * l0);
            vk += nao2;
        }
        dm += nao2;
    }
...
__global__ static void GINTint2e_jk_kernel1000(JKMatrix jk,
    BasisProdOffsets offsets, GINTEnvVars envs, BasisProdCache bpcache) {
    int ntasks_ij = offsets.ntasks_ij;
    long ntasks = ntasks_ij * offsets.ntasks_kl;
    long task_ij = blockIdx.x * blockDim.x + threadIdx.x;
